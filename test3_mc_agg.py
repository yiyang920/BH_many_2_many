...
m2m_data_loc = r"many2many_data\\FR_and_2\\"
build_on = {4: "new_fr_1_agg.csv"}
...
G_d = pickle.load(open(r"Data\temp\G_d_agg.p", "rb"))
...
transit_trips_dict_pk_agg = dict()
for od, trips in transit_trips_dict_pk.items():
    if (disagg_2_agg_id[od[0]], disagg_2_agg_id[od[1]]) in transit_trips_dict_pk_agg:
        transit_trips_dict_pk_agg[
...
trip_dict = {
    ...
    for k, v in transit_trips_dict_pk_agg.items()
...
Rider.to_csv(m2m_data_loc + r"Rider_agg.csv", index=False)
Rider = Rider.to_numpy(dtype=int, na_value=999)

# Generate driver information
# Load fixed routed infomation
if config["FIXED_ROUTE"]:
    df_blue_1 = pd.read_csv(m2m_data_loc + r"blue_fr_1_agg.csv")
    df_blue_2 = pd.read_csv(m2m_data_loc + r"blue_fr_1_agg.csv")
    df_red_1 = pd.read_csv(m2m_data_loc + r"red_fr_1_agg.csv")
    df_yellow_1 = pd.read_csv(m2m_data_loc + r"yellow_fr_1_agg.csv")
...
if config["BUILD_ON"]:
    if not isinstance(FR, dict):
        FR = {}
    for idx, fname in build_on.items():
        FR[idx] = pd.read_csv(m2m_data_loc + fname).to_numpy()


if config["REPEATED_TOUR"]:
    Driver = pd.read_csv(m2m_data_loc + r"Driver_rt_agg.csv")
else:
    Driver = pd.read_csv(m2m_data_loc + r"Driver_agg.csv")
...
# %% Store results
result_loc = r"test_data\result\3_28_2021\\"
